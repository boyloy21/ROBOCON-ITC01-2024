...
from farmer_robot.Omni_Farmer import Omni_model
import math
import time
...
class Farmer_PID(Node):
    def __init__(self):
        ...
        self.feedback_sub = self.create_subscription(Float32MultiArray, '/current', self.External_feedback, 10)
        ...
        self.goal3 = [2.0, -3.5, -1.57] 
        self.goal_end = [0.0, 0.0, 0.0]
        self.laserX_refX = [3.33, 1.5, 1.0, 3.38]  
        self.laserY_refY = [3.0, 2.0, 1.0]
        # self.laserX_refY = []
        # self.laserY_refX = []
        # SENSOR FEEDBACK   
        self.current_x = -0.0
    ...
    def Encoder_feedback(self, W):
        ...
        self.W3 = W.data[2]
        self.W4 = W.data[3]
        self.V = self.omni.forward_kinematic(self.W1, self.W2, self.W3, self.W4, self.theta, "numpy")
        self.Vx_M = self.V[0]
        self.Vy_M = self.V[1]
    ...
    def Calculate_position(self):
        ...
        self.current_x = self.current_x + self.Vx_M*self.dt
        self.current_y = self.current_y + self.Vy_M*self.dt
        self.current_yaw = self.theta
    ...
    def Calculate_goal(self):
        ...
        # TAKE RICE FIRST
        if (self.Position == 1 ): 
            error_laserX = self.laserX - self.laserX_refX[0]
            error_laserY = self.laserY_refY[0] - self.laserY
            if (self.state == 0):
                self.goal_end = self.goal1
                self.state = 1
            elif (abs(errorX)<=0.1 and self.state == 1):
        ...
        # PLANT RICE FIRST
            ...
            error_laserX = self.laserX - self.laserX_refX[1]
            error_laserY = self.laserY_refY[1] - self.laserY
            if (abs(error_laserX)>0.03 and abs(error_laserY)>0.03 and self.state == 3):
                self.goal_end = [error_laserX+self.current_x, error_laserY+self.current_y, 0.0]
            elif (abs(error_laserX)<=0.03  and abs(error_laserY)<=0.03 and self.state ==3):
                self.goal_end = [self.current_x, self.current_y, 0.0]
                self.state = 4
        
        # PLANT RICE SECOND
        elif (self.Position == 3):
            error_laserX = self.laserX - self.laserX_refX[2]
            error_laserY = self.laserY_refY[1] - self.laserY
            if (abs(error_laserX)>0.03 and abs(error_laserY)>0.03 and self.state == 4):
                self.goal_end = [error_laserX+self.current_x, error_laserY+self.current_y, 0.0]
            elif (abs(error_laserX)<=0.03  and abs(error_laserY)<=0.03 and self.state ==4):
                self.goal_end = [self.current_x, self.current_y, 0.0]
                self.state = 5

        # TAKE RICE AGAIN (Second)
        elif (self.Position == 4):
            error_laserX = self.laserX - self.laserX_refX[0]
            error_laserY = self.laserY_refY[2] - self.laserY
            if (abs(error_laserY)>0.03 and self.state == 5):
                self.goal_end = [0.2+self.current_x, error_laserY+self.current_y, 0.0]
                self.state = 6
            if (abs(errorY)<0.05 and abs(error_laserX)>0.03 and abs(error_laserY)>0.03 and self.state == 6):
                self.goal_end = [error_laserX+self.current_x, error_laserY+self.current_y, 0.0]
            elif (abs(error_laserX)<=0.03  and abs(error_laserY)<=0.03 and self.state ==6):
                self.goal_end = [self.current_x, self.current_y, 0.0]
                self.state = 7
        
        # PLANT RICE AGAIN (THIRD)
        elif (self.Position == 5):
            error_laserX = self.laserX - self.laserX_refX[1]
            error_laserY = self.laserY_refY[2] - self.laserY
            if (abs(error_laserX)>=0.03 and abs(error_laserY)>=0.03 and self.state == 7):
                self.goal_end = [self.current_x+error_laserX, self.current_y+error_laserY, 0.0]
            elif (abs(error_laserX)<0.03 and abs(error_laserY)<0.03 and self.state == 7):
                self.goal_end = [self.current_x, self.current_y, 0.0]
                self.state = 8

        # DROP RICE AGAIN (FOURTH)
        elif (self.Position == 6):
            error_laserX = self.laserX - self.laserX_refX[1]
            error_laserY = self.laserY_refY[3] - self.laserY
            ...
            error_laserX = self.laserX_refX[3] + self.laserY
