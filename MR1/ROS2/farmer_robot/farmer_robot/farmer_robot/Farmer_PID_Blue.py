...
class Farmer_PID(Node):
    def __init__(self):
        ...
        self.goal1 = [0.55, 0.0, 0.0]
        self.goal2 = [0.55, 1.38, 0.0]
        self.goal3 = [2.25, 3.0, 1.57]
        self.goal_end = [0.0, 0.0, 0.0]
        self.laserX_ref = [-3.29, -3.3, -3.3, 1.725]
        self.laserY_ref = [-2.58, -1.55, -0.823, 1.17, 1.68, 2.156, 2.66, 3.1, 3.60]
        self.motor_pos = [-0.66, 0.55, -0.55, 0.35, -0.35, 0.2]
        self.motor_col = 0.0
        # SENSOR FEEDBACK
        self.current_x = -0.0
        self.current_y = 0.0
    ...
    def Calculate_goal(self):
        ...
        if (self.Position == 1 ):
            error_laserX = self.laserX_ref[0] + self.laserX
            error_laserY = self.laserY_ref[0] + self.laserY
            if (self.state == 0):
                ...
                self.state = 2
            elif (abs(errorY)<=0.1 and abs(error_laserX)>0.05 and abs(error_laserY)>0.05  and self.state == 2):
            ...
            error_laserX = self.laserX_ref[1] + self.laserX
            error_laserY = self.laserY_ref[1] + self.laserY
            ...
            error_laserX = self.laserX_ref[2] + self.laserX
            error_laserY = self.laserY_ref[2] + self.laserY
            ...
            error_laserX = self.laserX_ref[3] - self.laserY
            error_laserY = -self.laserY_ref[3] + self.laserX
            if (self.state == 5):
            ...
            error_laserX = self.laserX_ref[3] - self.laserY
            error_laserY = -self.laserY_ref[4] + self.laserX
            ...
            error_laserX = self.laserX_ref[3] - self.laserY
            error_laserY = -self.laserY_ref[5] + self.laserX
            ...
            error_laserX = self.laserX_ref[3] - self.laserY
            error_laserY = -self.laserY_ref[6] + self.laserX
            ...
            error_laserX = self.laserX_ref[3] - self.laserY
            error_laserY = -self.laserY_ref[7] + self.laserX
            if (abs(error_laserX)>0.03 and abs(error_laserY)>0.03 and self.state == 9):
                self.goal_end = [self.current_x+error_laserX, self.current_y+error_laserY, 1.57]
            elif (abs(error_laserX)<=0.03 and abs(error_laserY)<=0.03 and self.state == 9):
                self.goal_end = [self.current_x, self.current_y, 1.57]
            ...
            error_laserX = self.laserX_ref[3] - self.laserY
            error_laserY = -self.laserY_ref[8] + self.laserX
        ...
        #         self.goal_end = [self.current_x, self.current_y, -1.57]
        #         self.state = 12
        self.get_logger().info('Position: %d, State: %d, Motor_pos: %f' %(self.Position, self.state,self.motor_col))
        ...
        if (self.Position == 1 and self.state ==1):
            ...
            self.motor_col = self.motor_pos[0]
            self.position_motor_pub.publish(motor)
        elif (self.Position == 2 and self.state == 3):
            ...
            self.motor_col = self.motor_pos[0]
            self.position_motor_pub.publish(motor)
        elif (self.Position == 3 and self.state == 4):
