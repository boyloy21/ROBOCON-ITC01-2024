...
class Buffalo_PID(Node):
    def __init__(self):
        ...
        # GOAL FIRST START
        self.goal0 = np.array([6.2, -0.0, 0.0])
        self.goal1 = np.array([6.2, 3.3, 0.0])
        self.goal2 = np.array([9.0, 3.3, 0.0])
        self.goal3 = np.array([9.8, 1.5, -1.57])
        self.goal4 = np.array([9.8, 1.5, -1.57])
        
        # GOAL RETRY START
        self.goal0_retry = [0.85, 1.2, 0.0]
        self.goal1_retry = [0.85, 3.5, 0.0]
        ...
        self.laser_bridgeY = 1.43 # when battery full is 1.65
        self.silo_rnd1 = [1,2,3]
        self.silo_rnd2 = [3,4,5]
        self.silo3 = 0
        self.laserX = 0.0
        self.lasery = 0.0
    ...
    def Calculate_position(self):
        ...
        elif(((self.state >= 6 and self.state<=12) or (self.state>=17 and self.state<=18))):
        ...
        # When Find silo
        elif((self.state >= 13 and self.state<=16)):
            self.pid_X= PIDController(1.2, self.Ki[0], self.Kd[0], self.dt, self.alpha, -1.0, 1.0, -1.0, 1.0)
            self.pid_Y= PIDController(1.2, self.Ki[1], self.Kd[1], self.dt, self.alpha, -1.0, 1.0, -1.0, 1.0)
    ...
    def calculate_goal(self):
        ...
        if (self.silo3 == 3 ):
            self.silo_rnd1 = [1,2]
            self.silo_rnd2 = [4,5]
        if ( self.check_three_ball == 0 and self.state == 14 ):
            self.state = 15
        elif (self.check_three_ball ==1 and self.state == 14 ):
            self.silo -=1
            if (self.silo == 0 ):
                self.silo = 5
            if (self.silo == 3):
        ...
        # WHEN ROBOT CATCH BALL
            ...
            self.take_ball = 0
            self.state_yaw = 0
        elif ((self.distance<=0.40 and self.distance>0.3) and (curr_timeball-self.prev_time)>3.0 and self.state == 5 and self.mode_status == "auto"):
            self.mode_status = "manual"
            self.Omega = -1.5
            self.state = 20
        ...
        # WHEN ROBOT TAKE BALL AREADY
            ...
            if (self.current_x >= self.goal_limitball[0] and self.current_x <= self.goal_limitball[0] +2.5):
                self.state_end = [self.current_x+0.6, self.current_y, -0.75]
            else:
                self.state_end = [self.current_x-0.6, self.current_y, -0.75]
            self.state = 7
        elif (abs(self.errorx)<=0.2 and abs(self.erroryaw)<=0.1 and self.state == 7):
            self.state_end = [self.current_x, self.current_y+2.5, -0.75]
            self.state = 8
        elif (abs(self.errory)<=0.3 and self.state == 8):
            self.state_end = [self.current_x, self.current_y, -1.57]
            self.state = 10
        # WHEN ROBOT CHECK LASER TO MOVE SILO POSITION_X IN ROBOT NAER
        # elif (abs(self.erroryaw)<=0.1 and abs(self.errory)<=0.1 and not((self.laserX>=self.laser_refxwall[0] and self.laserX<=self.laser_refxwall[1])) and self.state == 10):
        ...
        #     self.state = 11
        # elif (abs(self.erroryaw)<=0.1 and abs(self.errory)<=0.1 and (self.laserX>=self.laser_refxwall[0] and self.laserX<=self.laser_refxwall[1]) and self.state == 10):
        ...
        elif (abs(self.erroryaw)<=0.1 and abs(self.errory)<=0.1  and self.state == 10):
            if (self.laserY >= self.laser_refysilo[3]):
                # silo = [1,2,3]
        ...
        elif ((self.state == 12 and (not(self.laserX >= 0.01 and self.laserX <= 1.4)))):
            self.state_end = [self.current_x, self.current_y+1.0, -1.57]
            self.state == 13
       
        #  CHECK  Threee Ball
        elif ((self.laserX >= 0.001 and self.laserX <= 1.4) and (self.state == 13 or self.state==12)):
        ...
        # WHEN ROBOT CHECK POSITION_Y AND SHOOT BALL TO SILO
        elif ((self.laserX >= 0.01 and self.laserX <= 2.2)  and (self.state == 15)):
            ...
            if ((abs(error_laserx_py)>=0.05 and (abs(error_lasery_px)>=0.04)) and self.state == 15):
                self.state_end = [self.current_x+error_lasery_px, error_laserx_py+self.current_y, -1.57]
            elif ((abs(error_laserx_py)<0.05 and (abs(error_lasery_px)< 0.04)) and (self.state == 15)):
                self.state_end = [self.current_x+error_lasery_px, error_laserx_py+self.current_y, -1.57]
                self.state = 17
        ...
        elif ((self.laserX<=2.1 and self.laserX>=1.7) and self.state == 18 ):
                errorsilo3 = self.laserY - self.laser_refysilo[3]
                self.state_end = [self.current_x+errorsilo3, self.current_y-2.5, -1.57]
                self.state = 19
        elif (abs(self.errory)<=0.5 and self.state == 19):
                self.state = 5
                self.take_ball = 0
                self.state_move = 0
                self.state_yaw = 0
