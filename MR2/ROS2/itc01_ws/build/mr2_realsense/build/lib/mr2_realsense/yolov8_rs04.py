...
class YOLODetector(Node):
    ...
    def __init__(self):
        ...
        self.model = YOLO(os.path.join(os.environ['HOME'], 'yolov8_rs', 'ball_yolov8t03.pt'))
    ...
    def detect_objects(self):
        ...
        for r in results:
            ...
            for box in boxes:
                ...
                # x_object, y_object, z_object = self.pixel_to_3d(x_center, y_center, depth, intrinsics)
                # angle = np.arcsin(z_object/np.sqrt(x_object**2+y_object**2+z_object**2))/2
                ...
                if int(c) == 1:
                    ...
                    object_distance.data = [depth,yaw]
                    rotate = -1*yaw*np.pi/180
                    
                    self.ball_distance_publisher.publish(object_distance)

                    self.get_logger().info('distance :%f,radian : %f, degree: %f]'%(depth,rotate,-1*yaw))
