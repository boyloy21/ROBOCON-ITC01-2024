...
class Buffalo_PID(Node):
    ...
    def Calculate_position(self):
        ...
        if ((self.state == 5)):
            ...
            self.pid_Yaw= PIDController(1.8, self.Ki[2], self.Kd[2], self.dt, self.alpha, -1.2, 1.2, -1.2, 1.2)
    ...
    def calculate_goal(self):
        ...
        #When NOT SEE BALL
            ...
            if (self.current_x >= self.goal_limitball[0] and self.current_x <= self.goal_limitball[1] -3.5):
        ...
        # WHEN ROBOT CHECK LASER TO MOVE SILO POSITION_X IN ROBOT NEAR
            ...
            self.state = 11
        # elif (abs(self.erroryaw)<=0.1 and abs(self.errory)<=0.1 and not((self.laserX>=self.laser_refxwall[0] and self.laserX<=self.laser_refxwall[0])) and self.state == 10):
        #     if (self.laserY >= self.laser_refysilo[3]):
        #         # silo = [1,2,3]
        #         self.silo = np.random.choice(self.silo_rnd1)
        #         if (self.silo == 3):
        ...
        # WHEN ROBOT CHECK POSITION_Y AND SHOOT BALL TO SILO
            ...
            if ((abs(error_laserx_py)>=0.04 and (abs(error_lasery_px)>=0.05)) and self.state == 15):
                self.state_end = [self.current_x+error_lasery_px, error_laserx_py+self.current_y, 1.57]
            elif ((abs(error_laserx_py)<0.04 and (abs(error_lasery_px)< 0.05)) and (self.state == 15)):
                self.state_end = [self.current_x+error_lasery_px, error_laserx_py+self.current_y, 1.57]
                self.state = 17
                ...
                self.goaltake_ball = [self.current_x+errorsilo3,-1.8, 1.57]
                self.state_end = self.goaltake_ball
                self.state = 18
