...
from buffalo_robot.Omni_model import OmniModel
...
class PID_Simulation(Node):
    def __init__(self):
        ...
        self.feedback_sub = self.create_subscription(Twist, '/back', self.subscrib_feedback, 10)
        ...
        self.Lx = 0.23
        self.Ly = 0.23 
        self.omni=OmniModel(self.R,self.Lx,self.Ly)
        self.sampling_time = 0.01
        self.mode_status = "manual"

        ## Variable PID
        self.dt = 0.01
        self.Kp = np.array([2.5, 2.5, 1.4])
        self.Ki = np.array([0.001, 0.001, 0.001])
        self.Kd = np.array([0.008, 0.008, 0.002])
        self.alpha = 0.7
    ...
    def subscrib_feedback(self, back):
        self.measurement_x = back.linear.x
        self.measurement_y = back.linear.y
        self.measurement_yaw = back.angular.z
    def Calculate_position(self):
        ...
        W = self.omni.Inverse_kinematic(self.output_vx,self.output_vy,self.output_omega)
        x_next,y_next,yaw_next = self.omni.discrete_state(self.current_x, self.current_y, self.current_yaw,W[0,0],W[1,0],W[2,0],W[3,0],self.sampling_time)
        self.current_x = x_next
        self.current_y = y_next
        self.current_yaw = yaw_next
        
        self.hist_x.append(self.current_x)
        self.hist_y.append(self.current_y)
        self.hist_yaw.append(self.current_yaw)
        plt.clf()
        plt.gcf().canvas.mpl_connect('key_release_event',
                        lambda event: [exit(0) if event.key == 'escape' else None])
        plot_arrow(self.current_x, self.current_y, self.current_yaw)
        plt.plot(self.state_end[0], self.state_end[1], marker="x", color="blue", label="Input Trajectory")
        plt.plot(self.hist_x, self.hist_y, color="blue", label="PID Track")
        plt.xlabel('X Position')
        plt.ylabel('Y Position')
        plt.title("Velocity of robot [m/sec]:" + str(round(self.output_vx,2)))
        plt.axis("equal")
        plt.grid(True)
        plt.legend()
        plt.pause(0.0001)
