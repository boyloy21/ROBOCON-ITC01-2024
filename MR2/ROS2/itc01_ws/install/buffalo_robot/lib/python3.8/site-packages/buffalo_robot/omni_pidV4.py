...
class Buffalo_PID(Node):
    def __init__(self):
        ...
        self.ball_count_pub = self.create_subscription(Int8MultiArray, '/total_ball_count', self.ball_count_callback, 10)
        ...
        self.goal0 = [6.0, 0.0, 0.0]
        self.goal1 = [6.0, -3.8, 0.0]
        self.goal2 = [9.65, -3.8, 0.0]
        self.goal3 = [9.65, -1.5, 1.57]
        self.goal4 = [9.65, -1.5, 1.57]
        self.goalball = [0.0,0.0,0.0]
        # self.silo1 = [8.15, -4.8, 1.57]
        # self.silo2 = [8.9, -4.8, 1.57]
        # self.silo3 = [9.65, -4.8, 1.57]
        # self.silo4 = [10.4, -4.8, 1.57]
        # self.silo5 = [11.15, -4.8, 1.57]
        # self.ball1 = [8.5, -1., 1.57]
        # self.ball2 = [10.0, -1. , 1.57]
        # self.ball3 = [11.0, -1. , 1.57]
        self.goal_limitball = [0.0, 0.0]
        self.goaltake_ball = [9.65, -2.0, 1.57]
        # When Retry
        self.goal0_retry = [0.8, -1.2, 0.0]
        self.goal1_retry = [0.8, -3.8, 0.0]
        self.goal2_retry = [4.475, -3.8, 0.0]
        ...
        # PARAMETER LASER
        self.laser_bridgeY = 1.25
        self.laser_refxsilo = -0.015
        self.laser_refysilo = [0.0, 2.86, 2.20, 1.54, 0.89]
    ...
    def ball_count_callback(self, count):
        ...
        if (self.laserX>=0.6 and self.laserX<=1.12 and self.state == 9):
            if (self.silo == 1):
    ...
    def ball_callback(self, ball):
        if (self.typeball == "no_ball"):
            ...
            yaw = -1*ball.data[1]*np.pi/180 + 0.02
    ...
    def Calculate_position(self):
        ...
        if (self.current_x >= self.goal_limitball[0] and self.current_x < self.goal_limitball[1] and self.current_y >= -4.5 and self.current_y < 0.5 and self.state == 5):
        ...
        elif(self.current_x >= self.goal_limitball[0] and self.current_x <= self.goal_limitball[1] and self.current_y >= -10.0 and self.current_y < 0.5 and self.state > 5) :
            self.pid_X= PIDController(2.2, self.Ki[0], self.Kd[0], self.dt, self.alpha, -1.3, 1.3, -1.3, 1.3)
            self.pid_Y= PIDController(2.5, self.Ki[1], self.Kd[1], self.dt, self.alpha, -1.3, 1.3, -1.3, 1.3)
    ...
    def calculate_goal(self):
        ...
        #When NOT SEE BALL
        ...
        elif ((self.current_y >= -4.5)  and (curr_timeball - self.prev_timeball)>3.0 and self.state == 5  and  self.typeball == "no_ball" and self.state_yaw ==0 ):
            self.mode_status = "manual"
            self.Omega = -1.2
            self.state_yaw = 1
            self.state_count = 0
            ...
            self.Omega = 1.2
            self.prev_timeball = curr_timeball
        elif ((self.current_yaw>= 2.8 and self.current_yaw <= 3.14) and self.mode_status == "manual" and  self.typeball == "no_ball" and self.state == 5 and (curr_timeball - self.prev_timeball)>0.5 and self.state_yaw == 1): 
            self.mode_status = "manual"
            self.Omega = -1.2
            self.state_count +=1
            self.prev_timeball = curr_timeball
        elif (self.state_count == 2 and self.state == 5 and self.typeball == "no_ball" and self.state_move ==0):
            self.Omega = 0.0
            self.mode_status = "auto"
            self.state_end = [self.current_x+0.3, self.current_y+0.3, 1.57]
            self.state_move = 1
            self.state_yaw = 0
            self.state_count = 0
            ...
            self.state_yaw = 0
         # When Robot hit wall 
        # elif ((self.current_y >= 0.01 or (self.current_x >= self.goal_limitball[0] and self.current_x <= self.goal_limitball[0]+0.5) or (self.current_x >= self.goal_limitball[1]-0.7 and self.current_x <= self.goal_limitball[1])) and self.typeball == "no_ball" and (curr_timeball - self.prev_timeball)>1.2 and self.state == 5):
        #     self.mode_status = "auto"
        #     if (self.current_yaw>=-2.8 and self.current_yaw<=1.0 ):
        #         self.state_end  = [self.current_x-0.5,self.current_y,self.current_yaw]
        #     elif (self.current_yaw>=1.8 and self.current_yaw<=3.14):
        #         self.state_end  = [self.current_x+0.5,self.current_y,self.current_yaw]
        #     else:
        #         self.state_end = [self.current_x, self.current_y-0.5, self.current_yaw]
        #     self.state = 5
        #     self.prev_timeball = curr_timeball 
            
        # WHEN ROBOT TAKE BALL AREADY
        elif (self.state == 6 and self.mode_status == "auto"):
            if (self.current_x >= self.goal_limitball[0] and self.current_x <= self.goal_limitball[1] -2.5):
                self.state_end = [self.current_x+1.0, self.current_y-2.2, 1.57]
            elif ((self.laserY >=0.02 and self.laserY<=0.9 and self.current_x>=self.goal_limitball[1] -3.0) or (self.current_x>=self.goal_limitball[1]-2.0 and self.current_x<=self.goal_limitball[1])):
                self.state_end = [self.current_x-1.0, self.current_y-2.2, 1.57]
            else:
                self.state_end = [self.current_x, self.current_y-2.2, 1.57]
            self.state = 7
            self.state_yaw = 0
        
        # WHEN ROBOT CHECK LASER TO MOVE SILO POSITION_X IN ROBOT NEAR
        elif (abs(self.erroryaw)<=0.1 and abs(self.errory)<=0.1 and not((self.laserX>=self.laser_refxwall[0] and self.laserX<=self.laser_refxwall[0])) and self.state == 7):
            ...
            self.state = 8
        elif (abs(self.erroryaw)<=0.1 and abs(self.errory)<=0.1 and (self.laserX>=self.laser_refxwall[0] and self.laserX<=self.laser_refxwall[1]) and self.state == 7):
        ...
        # WHEN ROBOT START MOVE TO SILO
            ...
            if (abs(error_lasery)>= 0.2):
                ...
                self.state_end = [self.current_x+error_lasery, -4.8, 1.57]
                self.state = 9
            self.get_logger().info('Goal_Silo :"%d"' %self.silo)
        
        elif ((self.state == 9 and (not(self.laserX >= 0.01 and self.laserX <= 2.2)))):
            self.state_end = [self.current_x, self.current_y-1.0, self.current_yaw]
            self.state == 9
        # WHEN ROBOT CHECK POSITION_Y AND SHOOT BALL TO SILO
        elif ((self.laserX >= 0.01 and self.laserX <= 2.2)  and (self.state == 9)):
            ...
            if ((abs(error_laserx_py)>=0.005 and (abs(error_lasery_px)>=0.03)) and self.state == 9):
                self.state_end = [self.current_x+error_lasery_px, error_laserx_py+self.current_y, 1.57]
            elif ((abs(error_laserx_py)<0.005 and (abs(error_lasery_px)< 0.03)) and (self.state == 9)):
                self.state = 10
        ...
        elif ((self.laserX<=2.1 and self.laserX>=1.7) and self.state == 11 ):
                errorsilo3 = self.laserY - self.laser_refysilo[3]
                self.state_end = [self.current_x+errorsilo3, self.current_y+2.4, 1.57]
                self.state = 12
        elif (abs(self.errory)<=0.5 and self.state == 12):
                self.state = 5
                self.take_ball = 0
                self.state_move = 0
                self.state_yaw = 0
        ...
        self.get_logger().info('Goal_Silo :"%d", State: %d, goal_mode: %s' %(self.silo, self.state, self.typeball))
def main(args=None):
